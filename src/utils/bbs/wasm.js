...
console.log("WASM File Path789:", wasmPath);
let wasm;
...
async function loadWasm(url, imports = {}) {
    ...
    // 检查加载是否成功
    if (!response.ok) {
        throw new Error(`Failed to load WASM file at ${url}`);
    }
    ...
    // 返回实例的导出和模块本身
    return { wasm: instance.exports, module };
}
...
async function init(input) {
    ...
    // return wasm;
    try {
        // 调用 loadWasm 函数加载 WASM 文件
        console.log("input", wasmPath);
        const { wasm, module } = await loadWasm(wasmPath, imports);
